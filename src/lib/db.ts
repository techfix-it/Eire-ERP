...
// In Next.js, we use process.cwd() to get the project root
const dbPath = process.env.VERCEL ? "/tmp/techfix_v2.db" : path.join(process.cwd(), "techfix_v2.db");
...
export function initDb() {
  ...
  // Seeding from JSON files (path relative to server/data or project root)
  const seedDir = path.join(process.cwd(), "server", "data");
  
  seedTable("users", "users.json", seedDir, (data: any) => {
  ...
  seedTable("attribute_definitions", "attributes.json", seedDir, (data: any) => {
  ...
  seedTable("brands", "brands.json", seedDir, (data: any) => {
  ...
  seedTable("products", "products.json", seedDir, (data: any) => {
  ...
  seedTable("vehicles", "vehicles.json", seedDir, (data: any) => {
  ...
  seedTable("tasks", "tasks.json", seedDir, (data: any) => {
  ...
  seedTable("transactions", "transactions.json", seedDir, (data: any) => {
...
function seedTable(tableName: string, fileName: string, seedDir: string, seedFn: (data: any) => void) {
  const countRes = db.prepare(`SELECT COUNT(*) as count FROM ${tableName}`).get() as { count: number };
  if (countRes.count === 0) {
